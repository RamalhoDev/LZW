...
Report::Report(string instanceName,
			   int fileSize,
	...
	this->instanceName =
		instanceName.substr(0, instanceName.rfind(".")) + ".json";
}

void Report::writeReport() {
	ofstream file(this->instanceName, ios::out);

	string instance = this->instanceName.substr(
		instanceName.rfind("/") + 1,
		instanceName.rfind("_") - instanceName.rfind("/") - 1);
	...
	file << "\"compression_ratio\":"
		 << (double)this->fileSize / this->compressedFileSize << ",";
	file << "\"compression_ratio2\":"
		 << (double)this->fileSize /
				(this->dictionaryLength * this->indexBitLength / 8.0)
		 << ",";
	file << "\"execution_time\":" << seconds << ",";
	file << "\"used_all_indexes\":"
		 << ((dictionaryMaxSize == dictionaryLength) ? "true" : "false") << "}";
	file.close();
}
