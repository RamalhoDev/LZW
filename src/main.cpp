...
int main(int argc, char* argv[]) {
	...
	program.add_argument("--operation")
		...
		.default_value(std::string("compress"));
	program.add_argument("-o", "--output")
		.help("Output path.")
		.default_value(std::string("a.out"));
	program.add_argument("--dict_path")
		.default_value(std::string("a.dict"))
		.help("Read a dict file.");
	program.add_argument("--number_of_bits")
		.default_value(9)
		.help("Number of bits.")
		.scan<'i', int>();
	...
	// Third argument is the operation to execute (compress or decompress).
	// Default compress. Forth argument is the output path file.
	...
	if (operation == "compress") {
		...
		Report report = Report(outputPath, fileSize, size,
							   lzw.getIndexesLength(), K, seconds);
		report.writeReport();
	} else {
